# Module 08: Behaviour Records (4 Slugs) - UI Component & Screen Layout Extraction Specification
...
- **Target Module**: `Module 08: Behaviour Records (4 Slugs)`
...
- **Authoritative CRUD Reference**: [`references/08_...`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/references/)
...
### Screen 01: `assign-incident` (`media_1789154457434.png`)

- **Image File**: [`media_1789154457434.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789154457434.png)
- **Screen / Slug Name**: `assign-incident`
- **Target Platform Route**: `/super-admin/behaviour/assign-incident`
- **UI Layout Archetype**: **Archetype A: Split Two-Column Master-Detail Layout (Form Left / Table Right)**

#### 1. Extracted Text Content

- **Ground-Truth Screen Text Details**: Student point balance ledger, merit/demerit logging.

- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Assign Incident` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Assign Incident`.
...
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Assign Incident List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
...
### Screen 02: `incidents` (`media_1789154467543.png`)

- **Image File**: [`media_1789154467543.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789154467543.png)
- **Screen / Slug Name**: `incidents`
- **Target Platform Route**: `/super-admin/behaviour/incidents`
- **UI Layout Archetype**: **Archetype A: Split Two-Column Master-Detail Layout (Form Left / Table Right)**

#### 1. Extracted Text Content

- **Ground-Truth Screen Text Details**: Split 2-column taxonomy with positive/negative points and title.

- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Incidents` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Incidents`.
- **Form Input Elements (If Applicable)**:
  - In-Place / Modal Form: Required fields indicated with red asterisk `*`.
  - Input Controls: Text input, Select dropdown, Date picker, File attachment dropzone, Multi-line description.
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Incidents List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
  - Primary Columns: `ID / Code`, `Title / Name`, `Category / Group`, `Date`, `Status`, `Action`.
  - Status Badges: Standardized semantic status pills (`Active`, `Inactive`, `Pending`, `Approved`, `Paid`, `Unpaid`).
  - Action Buttons: Row-level tactical buttons: `View` (Details modal), `Edit` (Form populator), `Delete` (Confirmation prompt).
- **Pagination Controls**: `Showing 1 to 50 of Entries` with numbered pagination navigation buttons.
...
### Screen 03: `reports-setting` (`media_1789154487771.png`)

- **Image File**: [`media_1789154487771.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789154487771.png)
- **Screen / Slug Name**: `reports-setting`
- **Target Platform Route**: `/super-admin/behaviour/reports-setting`
...
- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Reports Setting` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Reports Setting`.
- **Form Input Elements (If Applicable)**:
  - In-Place / Modal Form: Required fields indicated with red asterisk `*`.
  - Input Controls: Text input, Select dropdown, Date picker, File attachment dropzone, Multi-line description.
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Reports Setting List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
  - Primary Columns: `ID / Code`, `Title / Name`, `Category / Group`, `Date`, `Status`, `Action`.
  - Status Badges: Standardized semantic status pills (`Active`, `Inactive`, `Pending`, `Approved`, `Paid`, `Unpaid`).
  - Action Buttons: Row-level tactical buttons: `View` (Details modal), `Edit` (Form populator), `Delete` (Confirmation prompt).
- **Pagination Controls**: `Showing 1 to 50 of Entries` with numbered pagination navigation buttons.
