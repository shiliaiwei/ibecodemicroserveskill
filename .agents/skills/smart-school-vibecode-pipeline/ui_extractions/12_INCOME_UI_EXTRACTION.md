# Module 12: Income (3 Slugs) - UI Component & Screen Layout Extraction Specification
...
- **Target Module**: `Module 12: Income (3 Slugs)`
...
- **Authoritative CRUD Reference**: [`references/12_...`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/references/)
...
### Screen 01: `add-income` (`media_1789155017093.png`)

- **Image File**: [`media_1789155017093.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789155017093.png)
- **Screen / Slug Name**: `add-income`
- **Target Platform Route**: `/super-admin/income/add-income`
- **UI Layout Archetype**: **Archetype A: Split Two-Column Master-Detail Layout (Form Left / Table Right)**

#### 1. Extracted Text Content

- **Ground-Truth Screen Text Details**: Split 2-column form (Income Head, Name, Invoice Number, Date, Amount, File Dropzone, Description).

- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Add Income` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Add Income`.
...
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Add Income List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
...
### Screen 02: `search-income` (`media_1789155027971.png`)

- **Image File**: [`media_1789155027971.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789155027971.png)
- **Screen / Slug Name**: `search-income`
- **Target Platform Route**: `/super-admin/income/search-income`
- **UI Layout Archetype**: **Archetype B: Top Filter Criteria Card + Full-Width Enterprise Data Table**

#### 1. Extracted Text Content

- **Ground-Truth Screen Text Details**: Dual period vs. keyword search, Chart of Accounts taxonomy.

- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Search Income` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Search Income`.
- **Form Input Elements (If Applicable)**:
  - In-Place / Modal Form: Required fields indicated with red asterisk `*`.
  - Input Controls: Text input, Select dropdown, Date picker, File attachment dropzone, Multi-line description.
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Search Income List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
  - Primary Columns: `ID / Code`, `Title / Name`, `Category / Group`, `Date`, `Status`, `Action`.
  - Status Badges: Standardized semantic status pills (`Active`, `Inactive`, `Pending`, `Approved`, `Paid`, `Unpaid`).
  - Action Buttons: Row-level tactical buttons: `View` (Details modal), `Edit` (Form populator), `Delete` (Confirmation prompt).
- **Pagination Controls**: `Showing 1 to 50 of Entries` with numbered pagination navigation buttons.
...
### Screen 03: `income-head` (`media_1789155038469.png`)

- **Image File**: [`media_1789155038469.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789155038469.png)
- **Screen / Slug Name**: `income-head`
- **Target Platform Route**: `/super-admin/income/income-head`
- **UI Layout Archetype**: **Archetype A: Split Two-Column Master-Detail Layout (Form Left / Table Right)**
...
- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Income Head` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Income Head`.
- **Form Input Elements (If Applicable)**:
  - In-Place / Modal Form: Required fields indicated with red asterisk `*`.
  - Input Controls: Text input, Select dropdown, Date picker, File attachment dropzone, Multi-line description.
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Income Head List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
  - Primary Columns: `ID / Code`, `Title / Name`, `Category / Group`, `Date`, `Status`, `Action`.
  - Status Badges: Standardized semantic status pills (`Active`, `Inactive`, `Pending`, `Approved`, `Paid`, `Unpaid`).
  - Action Buttons: Row-level tactical buttons: `View` (Details modal), `Edit` (Form populator), `Delete` (Confirmation prompt).
- **Pagination Controls**: `Showing 1 to 50 of Entries` with numbered pagination navigation buttons.
