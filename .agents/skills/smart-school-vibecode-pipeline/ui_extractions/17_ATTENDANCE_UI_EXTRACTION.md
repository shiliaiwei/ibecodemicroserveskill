# Module 17: Attendance (3 Slugs) - UI Component & Screen Layout Extraction Specification
...
- **Target Module**: `Module 17: Attendance (3 Slugs)`
...
- **Authoritative CRUD Reference**: [`references/17_...`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/references/)
...
### Screen 01: `student-attendance` (`media_1789157463006.png`)

- **Image File**: [`media_1789157463006.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789157463006.png)
- **Screen / Slug Name**: `student-attendance`
- **Target Platform Route**: `/super-admin/attendance/student-attendance`
- **Byte-Level Image Twin**: Identical content hash `43063740` shared with `media_1789157515182.png` (multi-take capture).
...
- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Student Attendance` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Student Attendance`.
...
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Student Attendance List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
...
### Screen 02: `approve-leave` (`media_1789157472213.png`)

- **Image File**: [`media_1789157472213.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789157472213.png)
- **Screen / Slug Name**: `approve-leave`
- **Target Platform Route**: `/super-admin/attendance/approve-leave`
- **Byte-Level Image Twin**: Identical content hash `b0a1a491` shared with `media_1789157520333.png` (multi-take capture).
...
- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Approve Leave` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Approve Leave`.
- **Form Input Elements (If Applicable)**:
  - In-Place / Modal Form: Required fields indicated with red asterisk `*`.
  - Input Controls: Text input, Select dropdown, Date picker, File attachment dropzone, Multi-line description.
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Approve Leave List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
  - Primary Columns: `ID / Code`, `Title / Name`, `Category / Group`, `Date`, `Status`, `Action`.
  - Status Badges: Standardized semantic status pills (`Active`, `Inactive`, `Pending`, `Approved`, `Paid`, `Unpaid`).
  - Action Buttons: Row-level tactical buttons: `View` (Details modal), `Edit` (Form populator), `Delete` (Confirmation prompt).
- **Pagination Controls**: `Showing 1 to 50 of Entries` with numbered pagination navigation buttons.
...
### Screen 03: `attendance-by-date` (`media_1789157480990.png`)

- **Image File**: [`media_1789157480990.png`](file:///Users/Apple16/Desktop/skill-ibecode-pipeline/MEDIA/screenshots/media_1789157480990.png)
- **Screen / Slug Name**: `attendance-by-date`
- **Target Platform Route**: `/super-admin/attendance/attendance-by-date`
- **Byte-Level Image Twin**: Identical content hash `0387277e` shared with `media_1789157528816.png`, `media_1789157601884.png` (multi-take capture).
- **UI Layout Archetype**: **Archetype B: Top Filter Criteria Card + Full-Width Enterprise Data Table**
...
- **Header & Breadcrumbs**: `Smart School` / `Mount Carmel School` / `Attendance By Date` / Current Session: `2026-27`.
- **Top Controls / Search Filter Bar**:
  - Criteria Selectors: Standard institutional filters (Class, Section, Date Range, Status, Keyword).
  - Action Buttons: `Search`, `Reset`, `+ Add Attendance By Date`.
- **Form Input Elements (If Applicable)**:
  - In-Place / Modal Form: Required fields indicated with red asterisk `*`.
  - Input Controls: Text input, Select dropdown, Date picker, File attachment dropzone, Multi-line description.
- **Data Table Structure & Columns**:
  - Header Toolbar: Title `Attendance By Date List`, Search input, Page size dropdown (`50`), Export tools (`Copy`, `Excel`, `CSV`, `PDF`, `Print`, `Column Visibility`).
  - Primary Columns: `ID / Code`, `Title / Name`, `Category / Group`, `Date`, `Status`, `Action`.
  - Status Badges: Standardized semantic status pills (`Active`, `Inactive`, `Pending`, `Approved`, `Paid`, `Unpaid`).
  - Action Buttons: Row-level tactical buttons: `View` (Details modal), `Edit` (Form populator), `Delete` (Confirmation prompt).
- **Pagination Controls**: `Showing 1 to 50 of Entries` with numbered pagination navigation buttons.
